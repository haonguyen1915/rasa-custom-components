...
class CustomeIntentClassifier(Component):
    ...
    def train(
        ...
        # viz_training_data(training_data, limit=10)
        # print(labels)
        # exit()
        if len(set(labels)) < 2:
            pass
            ...
            # print("inten sampke: {}".format(training_data.intent_examples[0]))
            # viz_message(training_data.intent_examples[0])
            # exit()
            # print("iDENSE_FEATURE_NAMES: {}".format(training_data.intent_examples[0].get(DENSE_FEATURE_NAMES[TEXT])))
            # print("sequence_to_sentence_features: {}".format(sequence_to_sentence_features(
            #     training_data.intent_examples[0].get(DENSE_FEATURE_NAMES[TEXT]))))
            # exit()
            # reduce dimensionality
            X = np.reshape(X, (len(X), -1))

            self.clf = self._create_classifier(num_threads, y)
            # viz_X_train_y_train(X, y)
            # exit()
